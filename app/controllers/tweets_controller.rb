class TweetsController < ApplicationController
  ...
  def reTweet
    ...
    isAlreadyTweeted=ReTweet.where("user_id=? and tweet_id=?", current_user.id, tweet.id)
    if tweet.user != current_user && isAlreadyTweeted.blank?
      ReTweet.create(:user_id => current_user.id, :tweet_id => tweet.id)
      redirect_to root_path
    else
      if !isAlreadyTweeted.blank?
        isAlreadyTweeted.first.destroy
      end
      redirect_to :back
    end
  end
